...
/// Build-in shaders
const std::string Renderer::_globalHeader = R"(
#version 330 core
...
int Renderer::CreateShaderHandler(std::shared_ptr<Shader> shader) {
    ...
    std::string vSource = _globalHeader + _vertexHeader + shader->GetVertexSource();
    std::string fSource = _globalHeader + _fragmentHeader + shader->GetFragmentSource();
